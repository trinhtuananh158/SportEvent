...
class MatchUpcomingAdapter(
    private var context: Context,
    private var matches: Matches,
    private val onItemClick: (MatchItem) -> Unit
) : RecyclerView.Adapter<MatchUpcomingAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        var match = matches.matches.upcoming[position]
    ...
    override fun getItemCount(): Int {
        return matches.matches.upcoming.size
    }

    fun updateMatches(newMatches: Matches) {
        matches.matches.upcoming.clear()
        matches = newMatches
